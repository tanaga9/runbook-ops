...
2. Draft the smallest useful procedure: purpose, scope, inputs, and Steps grouped by meaningful outcomes. Prefer verb + object headings; use Check / Act / Verify as completeness checks, not mandatory sections. Use `status: draft` while essential decisions remain unresolved. Keep commands visible; use inline Python or necessary adjacent support rather than an opaque workflow executor.
