...
- For long blocks, extract the original block into a temporary script using file tools and run it non-interactively instead of pasting into a terminal. Preserve guards, check syntax, and supply the working directory and inputs explicitly. Do not combine unrelated blocks or replay completed operations; remove temporary scripts afterward.
